...
export class TemplateButtonsBuilder {
    ...
    addURL(opts: { displayText: string, url: string }) {
        if(!opts.displayText || !opts.url) throw new Error('[whasapi] template button builder need url display text or url');
    ...
    addCall(opts: { displayText: string, phoneNumber: string }) {
        if(!opts.displayText || !opts.phoneNumber) throw new Error("[whasapi] template button builder need call display text or phone number");
    ...
    addQuickReply(opts: { displayText: string, id: string }) {
        if(!opts.displayText || !opts.id) throw new Error("[whasapi] template button builder need quick reply display text or id");

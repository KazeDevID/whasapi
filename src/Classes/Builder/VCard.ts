export class VCardBuilder {
    ...
    setFullName(fullName: string) {
        if(!fullName) throw new Error('[whasapi] vcard builder need full name')
        this.fullName = fullName;
        return this
    }

    setOrg(organizationName: string) {
        if(!organizationName) throw new Error("[whasapi] vcard builder need organization name");
        this.org = organizationName;
        return this
    }

    setNumber(number: string) {
        if(!number) throw new Error('[whasapi] vcard builder need number')
        this.number = number;
        return this
    }
